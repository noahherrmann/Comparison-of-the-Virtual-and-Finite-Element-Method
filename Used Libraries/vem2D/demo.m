...
switch opt
    case {1,5} 
        ...
        gradSol = @(x,y) deal(pi*cos(pi*x).*sin(pi*y), pi*sin(pi*x).*cos(pi*y) );
        if(opt == 1) % Poisson
        ...
        for indFile = 1:3 % mesh loop
            mesh = load(['./mesh/' files(indFile).name]);
            mesh = mesh.mesh;
            for indK = 1:numel(k)    % degree loop
                ...
                errH1(indFile, indK) = getH1Error(out,gradSol);
                % store diameter
        ...
        ylabel('L2 Error', 'FontSize', 16, 'FontWeight', 'bold')

        % plot H1 error as fn of k
        figure
        indMesh = 2;
        loglog(k,errH1(indMesh,:))
        xlabel('k', 'FontSize', 16, 'FontWeight', 'bold')
        ylabel('H1 Error', 'FontSize', 16, 'FontWeight', 'bold')
        
        % plot L2 error as fn of h
        figure
        colors = lines(numel(k)); 
        for indK = 1:numel(k)
            % Plot error
            loglog(diam(:,indK), errL2(:,indK), '-o', 'Color', colors(indK,:), ...
                'LineWidth', 1.5, 'DisplayName', ['k=' num2str(k(indK))]); 
            hold on
            
            % Anchor as starting point
            h_anchor = diam(end, indK);
            e_anchor = errL2(end, indK);
            h_ref = h_anchor * 4;
            p_slope = k(indK) + 1;
            e_ref = e_anchor * (h_ref / h_anchor)^p_slope;
            
            % dotted line below normal plot
            hLine = loglog([h_anchor, h_ref], [e_anchor, e_ref] * 0.6, 'k--', 'LineWidth', 1);
            set(get(get(hLine,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
            text(h_ref, e_ref * 0.6, [' h^{' num2str(p_slope) '}'], 'FontSize', 10);
        end
        
        xlabel('h (Mesh Diameter)', 'FontSize', 16, 'FontWeight', 'bold')
        ylabel('L2 Error', 'FontSize', 16, 'FontWeight', 'bold')
        title(['Convergence for ' meshType ' mesh'])
        lgd = legend('Location','NorthWest');
        lgd.FontSize = 18;
        lgd.FontWeight = 'bold'; grid on
        set(gca, 'XScale', 'log', 'YScale', 'log')
    case 2 % plot solution for a particular choice of h and k
        file = dir(['./mesh/' meshType '*_02.mat']);
        mesh = load(['./mesh/' file(1).name]);
        mesh = mesh.mesh;
        k = 4;
        ...
        colorbar
        xlabel('x', 'FontSize', 16, 'FontWeight', 'bold'), ylabel('y', 'FontSize', 16, 'FontWeight', 'bold'), title('Numerical solution for Poisson equation, $k=4$','Interpreter','Latex')
    ...
    case {3, 4} % mesh free solver for approximating harmonic functions
        ...
        loglog(k,error)
        xlabel('k', 'FontSize', 16, 'FontWeight', 'bold')
        ylabel('Error at one point', 'FontSize', 16, 'FontWeight', 'bold')
end
end
