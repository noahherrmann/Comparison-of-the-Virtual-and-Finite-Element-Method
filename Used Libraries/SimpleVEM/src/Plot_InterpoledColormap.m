function varargout = Plot_InterpoledColormap(coord, connectivityMatrix, value, varargin)
...
connectivityMatrix = [connectivityMatrix, connectivityMatrix(:,end)];
nnode = 4;
...
pntparent = [-1,-1 ; 1,-1 ; 1,1 ; -1,1];

for ielem=1:nelem
    [xi,eta] = meshgrid(-1:2/step:1 , -1:2/step:1);
    for i=1:length(eta)
        for j=1:length(xi)
            enne = feval("s2424_NN",pntparent,[xi(j,i),eta(j,i)]);
            for m=1:nnode
                NN(j,i,m) = enne(m);
            end
        end
