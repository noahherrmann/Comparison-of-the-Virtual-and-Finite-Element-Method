...
% -------------------------------------------------------------------------
p = 1;
tau_opt = 0.13;              % Example second tau value
tau_values = [1, tau_opt];     % Run simulations for these tau values
...
% -------------------------------------------------------------------------
% 4) Create the Geometries (Quarter Pipe)
% -------------------------------------------------------------------------
% We create the base geometry (for the Voronoi version) plus three
% quadrilateral geometries with different refinements.

[~, contour, options] = quarterPipe(1, 3, 6, 9, 50);
geometry = contour;

% Three quadrilateral geometries (vary the discretization parameters)
[geom1, contour1] = quarterPipe(1, 3, 10, 10, 20);
[geom2, contour2] = quarterPipe(1, 3, 15, 15, 20);
[geom3, contour3] = quarterPipe(1, 3, 20, 20, 20);
