function [soln,eqn,info] = PoissonP2(node,elem,bdFlag,pde_data,option)
...
% quadrature points
if ~isfield(pde_data,'d'), pde_data.d = []; end
if ~isfield(option,'quadorder')
    % diffusion is piecewise constant
    option.quadorder = 2;        % default order
    if ~isempty(pde_data.d) && isnumeric(pde_data.d) % numerical diffusion
...
for p = 1:nQuad
    ...
    for i = 1:6
        for j = i:6
            Aij = 0;
            if isempty(pde_data.d) || isnumeric(pde_data.d)
                Aij = Aij + w(p)*dot(Dphip(:,:,i),Dphip(:,:,j),2);
            else
                pxy = lambda(p,1)*node(elem(:,1),:) ...
                    + lambda(p,2)*node(elem(:,2),:) ...
                    + lambda(p,3)*node(elem(:,3),:);
                Aij = Aij + w(p)*dot(Dphip(:,:,i),Dphip(:,:,j),2).*pde_data.d(pxy);
            end
            if ~isempty(pde_data.d) && isnumeric(pde_data.d) % d is piecewise constant
                Aij = pde_data.d.*Aij;
            end
            Aij = Aij.*area;
...
if ~isfield(pde_data,'f') || (isreal(pde_data.f) && (pde_data.f==0))
    pde_data.f = [];
end
if ~isempty(pde_data.f) 
    ...
    for p = 1:nQuad
        ...
        pxy = lambda(p,1)*node(elem(:,1),:) ...
            + lambda(p,2)*node(elem(:,2),:) ...
            + lambda(p,3)*node(elem(:,3),:);
        if isfield(pde_data,'f') && isnumeric(pde_data.f)
            fp = pde_data.f;        % piecewise constant       
        else
            fp = pde_data.f(pxy);   % function handle
        end
        for j = 1:6
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~isfield(pde_data,'g_D'), pde_data.g_D = []; end
    if ~isfield(pde_data,'g_N'), pde_data.g_N = []; end
    if ~isfield(pde_data,'g_R'), pde_data.g_R = []; end
    ...
    if ~isempty(Robin) && ~isempty(pde_data.g_R) && ~(isnumeric(pde_data.g_R) && (pde_data.g_R == 0))
    ...
    if isempty(bdFlag) && ~isempty(pde_data.g_D) && isempty(pde_data.g_N) && isempty(pde_data.g_R)
        fixedDof = bdDof;
    ...
    if isempty(bdFlag) && (~isempty(pde_data.g_N) || ~isempty(pde_data.g_R))
    ...
    if ~isempty(pde_data.g_N) && ~isempty(Neumann) && ~(isnumeric(pde_data.g_N) && (pde_data.g_N == 0))
    ...
    if ~isPureNeumann && ~isempty(fixedDof) && ...
       ~isempty(pde_data.g_D) && ~(isnumeric(pde_data.g_D) && (pde_data.g_D == 0))
        idx = (fixedDof > N);                         % index of edge nodes
        u(fixedDof(~idx)) = pde_data.g_D(node(fixedDof(~idx),:)); % bd value at vertex dofs
        bdEdgeIdx = fixedDof(idx) - N;
        bdEdgeMid = (node(edge(bdEdgeIdx,1),:) + node(edge(bdEdgeIdx,2),:))/2;
        u(fixedDof(idx)) = pde_data.g_D(bdEdgeMid);
        b = b - A*u;
    end
