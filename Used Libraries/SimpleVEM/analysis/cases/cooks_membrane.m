...
p = 1;
tau_opt = 0.13;
tau_values = [1, tau_opt];  % We will run the simulation for these tau values
...
% -------------------------------------------------------------------------
% 4) Create the Geometries (Cook's Membrane)
% -------------------------------------------------------------------------
% For simplicity, we create the base geometry (for the Voronoi version) 
% and three quadrilateral geometries (nx, 2nx, 3nx).
[~, contour, options] = cookMembrane(4.4, 1.6, 2.0, 4.8, 12, 15);
geometry = contour;

nx = 15;
ny = 20;

% Quadrilateral geometries
[geom1, contour1] = cookMembrane(4.4, 1.6, 2.0, 4.8, nx,   ny);
[geom2, contour2] = cookMembrane(4.4, 1.6, 2.0, 4.8, 2*nx, 2*ny);
[geom3, contour3] = cookMembrane(4.4, 1.6, 2.0, 4.8, 3*nx, 3*ny);

% FEM: remove the first column of connectivity (typical for the FEM solver)
...
% VEM: same quadrilateral meshes, without removing extra columns
...
% (Optional) Check orientation of the Voronoi meshes
...
% Additional settings
options.p = p;
% If needed, specify the problem type:
% options.problemtype = 3; % Example (plane problem, plate, etc.)

% -------------------------------------------------------------------------
% 5) Material Definition
% -------------------------------------------------------------------------
mat = linearElastic2D_Steel;  % Replace with your own definition if needed

% -------------------------------------------------------------------------
% 6) MAIN LOOP over all tau values
% -------------------------------------------------------------------------
fprintf('[COOK MEMBRANE]\n');
for tau_idx = 1:length(tau_values)
    ...
    % ---------------------------------------------------------------------
    % 6.2) Pre-processing Plots and Boundary Conditions for Quad and Voronoi
    % ---------------------------------------------------------------------
    % -- QUAD 1
    ...
    % ---------------------------------------------------------------------
    % 6.4) Loop over the three mesh resolutions (n = 1:3)
    % ---------------------------------------------------------------------
    for n = 1:3
        fprintf('\n-------------------\n');
    ...
    % ---------------------------------------------------------------------
    % 6.5) Final Plot of the Energy Comparison
    % ---------------------------------------------------------------------
    Un = 197.94;
    figure;
    hold on;
    ...
    legend('FEM-quad', 'VEM-quad', 'VEM-voro', ...
        'Location','Best', 'FontSize', 14, 'Interpreter', 'latex', 'Location', 'best');
    ...
    fprintf('[COOK MEMBRANE]\n');
    % Close the diary for the current tau
    diary off;

    %% Plot Error reduction per DOF
    figure('Color', 'w');
    ...
    grid on; grid minor;
    title(['Convergence: Error vs DOFs ($\tau = ' num2str(current_tau) '$)'], 'FontSize', 18, 'Interpreter', 'latex');
