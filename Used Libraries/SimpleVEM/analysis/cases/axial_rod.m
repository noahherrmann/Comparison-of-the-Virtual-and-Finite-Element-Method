...
tau_values = [1];   % We will run simulations for these tau values
...
for tau_idx = 1:length(tau_values)
    ...
    % ---------------------------------------------------------------------
    % 6.5) Final Energy Comparison (Including Optional Analytical Lines)
    % ---------------------------------------------------------------------
    Un = 74.877;  % Example reference
    figure;
    hold on;
    ...
    % Uncomment if you want to plot the reference lines for each sample
